...
Three Automerge peers: Frontend WASM, Tauri relay, Daemon. The relay maintains its own `AutoCommit` and participates in two independent sync relationships (frontend↔relay, relay↔daemon). Every mutation goes through three merge operations. The relay's `receive_and_relay_sync_message` method uses the daemon peer state for frontend messages, which can corrupt sync state when `frontend_peer_state` is active.
...
No automatic reconnection. When the daemon restarts or the socket breaks, the frontend shows a banner and waits for the user to click reconnect. Each reconnect creates a new `NotebookSyncClient` and re-bootstraps the doc. `reset_sync_state()` on reconnect discards any unsynced local WASM mutations — users can see edits disappear.
...
3. Exchange Automerge sync messages until converged — this naturally merges any edits made during disconnection (preserving local changes instead of discarding them)
...
The `protocol` field in the handshake is optional and unvalidated. The server always responds with `v2` regardless of what the client requests. A client sending `protocol: "v99"` is silently treated as v2 with no warning.
...
Single-request-at-a-time per connection. Responses are correlated to requests by ordering: send request, wait for next Response frame. Works because the protocol is serial, but breaks if pipelining is ever needed. The `sync_to_daemon()` ack wait already demonstrates the fragility — it reads one frame and drops broadcasts that arrive before the ack.
...
No keepalive mechanism. Half-open connections (peer process killed) are only detected when the next write fails. The daemon's `active_peers` count can be wrong for the duration of the eviction delay. The `Relaxed` ordering on `active_peers` operations means stale values can be observed on ARM architectures.
...
The execution queue is a `VecDeque` guarded by implicit state: `executing: Option<String>` tracks the current cell, and `KernelStatus` is a flat enum. The iopub loop signals `ExecutionDone` when it sees an idle status, and `KernelDied` when the loop exits. Three independent death monitors (process watcher, iopub exit, heartbeat) provide redundant detection.

Known gaps: interrupt does not clear `executing` (relies on kernel sending idle); `try_send(ExecutionDone)` failures are discarded; QueueCommand dispatch is duplicated across auto-launch and manual-launch paths.
...
Kernel broadcasts use `tokio::broadcast(64)`. Slow peers get `Lagged` errors. After the `Lagged` fix, peers receive a full Automerge doc sync to catch up on persisted data (outputs in the CRDT). But ephemeral broadcasts (status changes, queue state) that aren't persisted to the doc are lost. `sync_to_daemon()` drops non-sync frames during ack waits — a separate source of lost broadcasts.
...
`active_peers` is incremented with `fetch_add(1)` after the handshake and decremented with `fetch_sub(1)` at function exit. If a panic occurs between increment and decrement, the count leaks. `Ordering::Relaxed` is used for all operations, which is insufficient for cross-thread visibility on ARM. The auto-launch check uses a separate `load()` instead of `fetch_add`'s return value, creating a TOCTOU race.
...
impl<'a> PeerGuard<'a> {
    fn new(room: &'a NotebookRoom) -> Self {
        let prev = room.active_peers.fetch_add(1, Ordering::AcqRel);
        Self { room }
    }

    fn is_first_peer(&self) -> bool {
        // Use fetch_add's return value (the previous count) to determine
        // if this was the first peer — eliminates TOCTOU with separate load()
...
impl Drop for PeerGuard<'_> {
    fn drop(&mut self) {
        let prev = self.room.active_peers.fetch_sub(1, Ordering::AcqRel);
        debug_assert!(prev > 0, "active_peers underflowed");
        if prev == 1 {
            // Schedule eviction
        }
    }
...
**Eviction generation counter**: Add an `AtomicU64` generation to `NotebookRoom`. Increment on every peer connect. The eviction task captures the generation at spawn time and aborts if it has changed, cleanly cancelling stale timers.
...
`doc.save()` runs inside the `room.doc.write()` lock on every incoming sync message. Serialization time grows with document size, blocking all other peers during serialization. The `format_notebook_cells` function also acquires the write lock per cell, allowing peer mutations to interleave between cells.
...
For `format_notebook_cells`, acquire a single write lock for the entire formatting operation to prevent interleaved mutations from producing a partially-formatted visible state.
...
1. **Two-peer sync** (Section 1) — Highest impact. Eliminates the largest correctness surface area. The relay's triple-merge path is the root cause of multiple audit findings (`sync_to_daemon` broadcast dropping, `receive_and_relay_sync_message` wrong peer state, three separate sync divergence surfaces).

2. **RAII peer guard** (Section 8) — Small, surgical. Eliminates peer count leak on panic, underflow risk, auto-launch TOCTOU, and stale eviction timer issues. Use `AcqRel` ordering for ARM correctness.

3. **Automatic reconnection** (Section 2) — High user impact. Daemon upgrades currently interrupt every open notebook. Must preserve unsynced local mutations (not discard via `reset_sync_state`).
...
5. **Broadcast state snapshots** (Section 7) — Completes the `Lagged` recovery story. The `Lagged` fix handles persistent data; this adds ephemeral state recovery.

6. **Execution state machine** (Section 6) — Formalizes the implicit state. Adds interrupt watchdog (prevents permanent queue stall when kernel ignores SIGINT). Eliminates the `try_send(ExecutionDone)` silent failure risk.

7. **Protocol versioning** (Section 3) — Low urgency today (one version), high value when the second version arrives.

8. **Request/response correlation** (Section 4) — Defensive measure. Also eliminates `sync_to_daemon` broadcast-dropping pattern by allowing interleaved responses and broadcasts.

9. **Persistence decoupling** (Section 9) — Performance, not correctness. Only matters for large notebooks. Also fixes per-cell write lock issue in formatting.
